...
MyBot::Environment::Environment(const std::vector<std::string>& fileNames)
{
    for (const std::string& fileName : fileNames)
    {
        ...
        for (const pugi::xml_node& language : document.child("localisation").children())
        {
            ...
            {
                if (_localisation.find(Language::English) == _localisation.end())
                    _localisation.insert(std::make_pair(Language::English, std::map<std::string, std::string>()));

                for (const pugi::xml_node& lexicalItem : language.children())
                    _localisation.at(Language::English).insert(std::make_pair(lexicalItem.name(), lexicalItem.child_value()));
            }
            else if (languageName == "russian")
            {
                if (_localisation.find(Language::Russian) == _localisation.end())
                    _localisation.insert(std::make_pair(Language::Russian, std::map<std::string, std::string>()));

                for (const pugi::xml_node& lexicalItem : language.children())
                    _localisation.at(Language::Russian).insert(std::make_pair(lexicalItem.name(), lexicalItem.child_value()));
            }
        }
    }
}

std::string MyBot::Environment::GetLexicalItem(MyBot::Language language, const std::string& key)
{
